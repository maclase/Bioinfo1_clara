...
# Procesamiento fasta de Chloroflexus ----------------------------------------

filename<-"ChloroflexusCDS" 
...
nombres_nuevos <- c()
for(i in 1:len){
  nombres_nuevos[i]<- paste("c",i,sep=".")
}
head(nombres_nuevos)
...
write.table(nombres_Neiss,quote=FALSE,row.names = FALSE,
            file=paste("nombres_Chloro",sep = "."),sep="\t")
